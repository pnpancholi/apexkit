...
export default function Home() {
  ...
  return (
    <>
        ...
        <div className={containerClasses}>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {featuresList.map((feature) => (
              <Card key={feature.title} icon={<feature.icon className="text-primary h-8 w-8" />} title={feature.title} desc={feature.description} hoverable />
            ))}
          </div>
        </div>
      </section>
      <section>
        <div className={containerClasses}>
          <h3 className={sectionHeaderClasses}>Built on the mordern ecosystem, loved by developers</h3>
          ...
          <div className="flex flex-wrap justify-center items-center gap-10 md:gap-16 opacity-80">
            {builtWithTools.map((tool) => (
              ...
              >
                <Image src={tool.imageURL} alt={tool.name} width={100} height={100} className="object-contain" />
